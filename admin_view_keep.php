        ...
        <div class="row">
            ...
            <main class="col-md-9 ml-sm-auto col-lg-10 px-md-4 py-4">
                ...
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 page-header">
                    <h1 class="h2"><i class="fas fa-edit text-info"></i> ใบลาที่ถูกลบ</h1>
                </div>
                            ...
                            <table class="table table-hover mb-0">
                                <thead>
                                    <tr>
                                        ...
                                        <th width="60" class="text-center">รีสไตร์</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php
                                    ...
                                    // Count total rows
                                    $sql_count = "select count(*) as total from absent1 A, staff B where A.STAFFID=B.STAFFID";
                                    $res_count = mysql_query($sql_count);
                                    $ln_count = mysql_fetch_array($res_count);
                                    $total_rows = $ln_count['total'];

                                    // Fetch data
                                    $sql = "select * from absent1 A, staff B where A.STAFFID=B.STAFFID order by date1 DESC limit $start, $limit";
                                    $res = mysql_query($sql);
                                    
                                    // Type Array
                                    $type_arr = array("","ลาป่วย","ลากิจ","ลาพักผ่อน","ไปราชการ","ลาคลอด","ลาอุปสมบท","ช่วยเหลือภริยาเลี้ยงดูบุตร","กักตัว (ผู้เสี่ยงโควิด-19)","ประกอบพิธีฮัจญ์");
                                    ...
                                    <tr>
                                        <td class="font-weight-bold">
                                            <?php echo $ln['PREFIXNAME'] . $ln['STAFFNAME'] . " " . $ln['STAFFSURNAME']; ?><br> <small><?=$ln[id]?></small>
                                        </td>
                                        <td>
                                        ...
                                        <td><span class="badge badge-light border"><?php echo $absent_type; ?></span></td>
                                        <td>
                                           <?php
                                            // HR Advise
                                        ...
                                        <td class="text-center"> 
                                            ...
                                            <a href="admin_store.php?id=<?php echo base64_encode($ln['id']); ?>" class="btn btn-warning action-btn text-white shadow-sm" title="รีสโตร์">
                        ...
                        <nav aria-label="Page navigation">
                            <ul class="pagination m-0" style="width:100%;overflow:scroll">
                                ...
                                if($start > 0) {
                                    ...
                                    echo "<li class='page-item'><a class='page-link' href='admin_view_keep.php?start=$prev_start'><i class='fas fa-chevron-left'></i></a></li>";
                                } else {
                                ...
                                for($i = 0; $i < $total_rows; $i += $limit){
                                    ...
                                    echo "<li class='page-item $active'><a class='page-link' href='admin_view_keep.php?start=$i'>$page_num</a></li>";
                                }

                                // Next
                                if(($start + $limit) < $total_rows) {
                                    $next_start = $start + $limit;
                                    echo "<li class='page-item'><a class='page-link' href='admin_view_keep.php?start=$next_start'><i class='fas fa-chevron-right'></i></a></li>";
                                } else {
